...
public class ObstacleCourse {
	...
	private boolean findExit(int row, int col) {
		...
		// If current row and col is possibility
		// Not possibility unless blank space or hasn't been tried.
		if (course[row][col] == ' ') {
			
			course[row][col] = TRIED; // set current location to tried

			if ((row == 0) || (row == course.length - 1) || (col == 0)
					|| (col == course[row].length - 1)) {// if on the border,
			
				foundRow = row;
				foundCol = col;	
				escaped = true; 	// The mover is out
			}

			else {

				escaped = findExit(row + 1, col); // below
