...
public class LinkedList<E extends Comparable<E>> {
	...
	public E get(int index) {
		...
		E el = getHelper(first, 0, index);
		return el;
	}

	private E getHelper(Node ref, int startIndex, int stopIndex) {

		// Base case--nothing to search for
		if (ref == null) {
			return null;
		}
		...
		if (startIndex == stopIndex) {
			...
			return getHelper(ref.next, startIndex + 1, stopIndex);
		}
	}
	...
	private void removeAllHelper(E target, Node ref) {
		...
		if (ref.next != null) {
			if (target.equals(ref.next.data)) {
				ref.next = ref.next.next;
				n--;
			}

			removeAllHelper(target, ref.next);
		}
	...
	private void dupHelper(Node ref, E target) {
		if (ref == null) {
			return;
		} else {
			if (target.equals(ref.data)) {
				ref.next = new Node(target, ref.next);
				n++;
			}

			dupHelper(ref.next.next, target);
